...
#include "juniper/afi_tree/afi_tree.pb.h"
#include "base64.h"
                                            ...
                                            Util::JsonArray* action_profiles)
{
    ...
    if (propv->expression->is<IR::ConstructorCallExpression>()) {
        ...
        if (implementationType->name == AFI::TableImplementation::actionSelectorName) {
            ...
            for (auto ke : key->keyElements) {
                ...
                BUG_CHECK(mt != nullptr, "%1%: could not find declaration", ke->matchType);
                if (mt->name.name != AFI::MatchImplementation::selectorMatchTypeName)
                    continue;
        ...
        } else if (implementationType->name == AFI::TableImplementation::actionProfileName) {
            isSimpleTable = false;
        ...
        if (type_extern_name == AFI::TableImplementation::actionProfileName) {
            table->emplace("type", "indirect");
        } else if (type_extern_name == AFI::TableImplementation::actionSelectorName) {
            table->emplace("type", "indirect_ws");
        } else {
            ::error("%1%: unexpected type for implementation", dcltype);
            return false;
                               ...
                               Util::JsonArray* action_profiles)
{
    ...
    auto afi_tree = new Util::JsonObject();

    juniper::afi_tree::AfiTree tree;

    afi_tree->emplace("afi-object-type", "afi-tree");
    ...
    afi_tree->emplace("afi-object-name", name);
    afi_tree->emplace("afi-object-id", nextId("tables"));


    ::ywrapper::StringValue *table_name = new ::ywrapper::StringValue();
    table_name->set_value(name);
    ...
    if (key != nullptr) {
        for (auto ke : key->keyElements) {
            ...
            if (match_type == AFI::MatchImplementation::selectorMatchTypeName)
                    continue;
            ...
            if (match_type != table_match_type) {
                if (match_type == AFI::MatchImplementation::rangeMatchTypeName)
                    table_match_type = AFI::MatchImplementation::rangeMatchTypeName;
    ...
    auto impl = table->properties->getProperty(AFI::TableAttributes::implementationName);
    bool simple = handleTableImplementation(impl, key, result, action_profiles);

    unsigned size = 0;
    auto sz = table->properties->getProperty(AFI::TableAttributes::sizeName);
    if (sz != nullptr) {
    ...
    if (size == 0)
        size = AFI::TableAttributes::defaultTableSize;
    ...
    table_size->set_value(size);
    tree.set_allocated_size(table_size);




    auto ctrs = table->properties->getProperty(AFI::TableAttributes::countersName);
    if (ctrs != nullptr) {
        ...
        if (ctrs->value->is<IR::ExpressionValue>()) {
            ...
            if (expr->is<IR::ConstructorCallExpression>()) {
                ...
                if (te->name != AFI::TableImplementation::directCounterName &&
                    te->name != AFI::TableImplementation::counterName) {
                    ::error("%1%: Unexpected type %2% for property", ctrs, type);
                    return result;
                }
                ...
                bool direct = te->name == AFI::TableImplementation::directCounterName;
    ...
    bool sup_to = false;
    auto timeout = table->properties->getProperty(AFI::TableAttributes::supportTimeoutName);
    if (timeout != nullptr) {
    ...
    auto dm = table->properties->getProperty(AFI::TableAttributes::metersName);
    if (dm != nullptr) {
        if (dm->value->is<IR::ExpressionValue>()) {
            ...
            if (!expr->is<IR::PathExpression>()) {
                ...
                auto te = type->to<IR::Type_Extern>();
                if (te->name != AFI::TableImplementation::directMeterName) {
                    ::error("%1%: Unexpected type %2% for property", dm, type);
                    return result;
                }
                if (!decl->is<IR::Declaration_Instance>()) {
...
bool ControlConverter::preorder(const IR::ControlBlock* block) {
    ...
    std::cout << "____________________ ControlConverter::preorder  ________________\n";
    ...
    std::cout << "______ block->container->name  ________________" << block->container->name << "\n";

    if (block->container->name == "ingress") {
        ::dump(block); 
    }
    ...
    auto cfg = new CFG();
    std::cout << "____________________ cfg->build________________\n";
    ...
    for (auto node : cfg->allNodes) {
        if (node->is<CFG::TableNode>()) {
            std::cout << "____________________ calling convertTable ________________\n";
            auto j = convertTable(node->to<CFG::TableNode>(), action_profiles);
            if (::errorCount() > 0)
                return false;
            std::cout << "____________________ tables->append  ________________\n";
            tables->append(j);
