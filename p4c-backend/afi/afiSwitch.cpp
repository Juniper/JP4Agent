...
#include "extensions/afi/backend.h"
#include "afiSwitch.h"

using AFI::mkArrayField;
using AFI::mkParameters;
using AFI::mkPrimitive;
using AFI::nextId;
using AFI::stringRepr;

namespace P4V1 {

// need a class to generate afiSwitch
...
SimpleSwitch::convertExternInstances(const IR::Declaration *c,
    ...
    if (eb->type->name == v1model.counter.name) {
        ...
        if (sz->to<IR::Constant>()->value == 0)
            error("%1%: direct registers are not supported in AFI", inst);
        ...
        if (eb->type->name == v1model.action_profile.name) {
            ...
            if (input == nullptr) {
                ...
                ::warning("Action selector '%1%' is never referenced by a table "
                        "and cannot be included in AFI JSON", c);
                return;
            }
