...
void AftTreeEntry::_bind()
{
    ...
    AftTreePtr aftTreePtr = std::dynamic_pointer_cast<AftTree>(AFIHAL::Afi::instance().getAfiObject(parent_name.value()));
    ...
    uint16_t portId = 2; // TBD: FIXME
    AftNodeToken outputPortToken = AftClient::instance().outputPortToken(portId);
    ...
    //jP4Agent->afiClient().addRoute(aftTreeToken, "1.1.1.1/10", etherEncapToken);
    Log(DEBUG) <<"Adding route...";
    for (unsigned int i =0; i < prefix_length.value(); i++) {
         std::cout << int(prefix_bytes_str[i]) << "." "\n"; 
    }

    AftClient::instance().addRoute(aftTreeToken, prefix_bytes_str.c_str(), prefix_bytes_str.size(), prefix_length.value(), etherEncapToken);
}

//  
// Description
//  
std::ostream & AftTreeEntry::description (std::ostream &os) const
{
