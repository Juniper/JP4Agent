...
class P4InfoResource
{
 ...
 public:
    P4InfoResource(const P4InfoResourceId id, const std::string &name,
                   const std::string &alias)
        : _id(id), _name(name), _alias(alias) {}
...
class P4InfoTable;
using P4InfoTablePtr =
    std::shared_ptr<P4InfoTable>;  ///< Pointer type of all P4Info resources
using P4InfoTableWeakPtr = std::weak_ptr<P4InfoTable>;

class P4InfoTable : public P4InfoResource
{
 public:
    explicit P4InfoTable(const p4::config::Table &table)
        : P4InfoResource(table.preamble().id(), table.preamble().name(),
    ...
    void display()
    {
        ...
        std::cout << "table.match_fields().size():"
                  << _table.match_fields().size() << std::endl;
        std::cout << "table.action_refs().size():"
                  << _table.action_refs().size() << std::endl;
 ...
 private:
    ...
    ///
    /// @brief           Define << class operator to append description to an
    /// output stream
    /// @param [in] os   Reference to output stream to append description to
    /// @param [in] p4infotable Reference to p4infotable to generate description
    /// for
    /// @returns         Reference to output stream with text description of
    /// p4infotable appended
    ///
    friend std::ostream &operator<<(std::ostream &        os,
                                    const P4InfoTablePtr &p4infotable)
    {
...
class P4InfoAction;
using P4InfoActionPtr =
    std::shared_ptr<P4InfoAction>;  ///< Pointer type of all P4Info actions
using P4InfoActionWeakPtr = std::weak_ptr<P4InfoAction>;

class P4InfoAction : public P4InfoResource
{
 public:
    explicit P4InfoAction(const p4::config::Action &action)
        : P4InfoResource(action.preamble().id(), action.preamble().name(),
    ...
    void display()
    {
        std::cout << "___ P4InfoAction ___" << std::endl;
    ...
    /// output stream
    /// @param [in] os   Reference to output stream to append description to
    /// @param [in] p4infoaction Reference to p4infoaction to generate
    /// description for
    /// @returns         Reference to output stream with text description of
    /// p4infoaction appended
    ///
    friend std::ostream &operator<<(std::ostream &         os,
                                    const P4InfoActionPtr &p4infoaction)
    {
...
{
 public:
    static P4Info &instance()
    {
        static P4Info p4info;
        return p4info;
    }

    P4Info(P4Info const &) = delete;
    P4Info(P4Info &&)      = delete;
    P4Info &operator=(P4Info const &) = delete;
    P4Info &operator=(P4Info &&) = delete;

    void insert2IdMap(const P4InfoResourcePtr &res) { _idMap[res->id()] = res; }
    ...
    const P4InfoResourcePtr p4InfoResource(const std::string &name)
    {
